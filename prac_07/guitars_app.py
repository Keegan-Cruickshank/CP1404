...
For something to do, pressing the guitar buttons changes
the cost in the guitar objects. Note we can either associate the
buttons with the guitar objects by name (then use name to find object in list)
or directly assign the object reference to the button
...
class GuitarsApp(App):
    ...
    def __init__(self, **kwargs):
        ...
        # - could be loaded from a file or something
    ...
    def create_widgets(self):
        ...
        :return: None
        """
        self.status_text = "Click on a guitar to reduce its cost by {:.1%}%"\
            .format(1 - DISCOUNT_RATE)
    ...
    def press_entry(self, instance):
        ...
        :return: None
        """
        # Each button was given its own ".guitar" object
        # reference, so we can get it directly
        guitar = instance.guitar
        old_cost = guitar.cost
        guitar.cost *= DISCOUNT_RATE
        # Update button text and label
        instance.text = str(guitar)
        self.status_text = "Your {} was ${:,.2f} but now costs ${:,.2f}"\
            .format(guitar.name, old_cost, guitar.cost)
